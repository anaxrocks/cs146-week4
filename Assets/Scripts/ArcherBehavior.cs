...
public class ArcherBehavior : MonoBehaviour
{
    ...
    void Update()
    {
        // If we don't have a player reference, nothing to do
        if (player == null) return;

        // Calculate distance once for efficiency
        float distanceToPlayer = calculateDistanceToPlayer();

        // Only update sprite facing when the player is reasonably near to save work
        if (distanceToPlayer <= range * 1.5f)
        {
            facePlayer();
        }

        // If player is in range and cooldown has finished, shoot
